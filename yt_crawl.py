...
playlists = [
    ['Q & A with GSD', 'PL9Zg64loGCGCFo5VXVb0KJrbiPhmOnGkK']
]
...
def run(api_key, gitCommit, datasetOutputLocation="dataset.json"):
    ...
    for video in output:
        ...
        title = video[2]
        # print(title)
        if title.endswith("with CC"):
            transcript = YouTubeTranscriptApi.get_transcript(vid)
            for cc in transcript:
                line = cc['text']
                # time = float(time)
                # hours, remainder = divmod(time, 3600)
                # minutes, seconds = divmod(remainder, 60)
                # minutes = ((cc['start'])*60) % 60
                minutes = 0
                seconds = math.floor(cc['start'])
                entry = SearchEntry(
                    title, video[1], minutes, seconds, tag, line).AsJsonSerializable()
                videos.append(entry)
            # tag = tags[title]
